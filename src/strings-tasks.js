...
function isPalindrome(str) {
  ...
  const reverseStr = newStr.split('').reverse().join('').replace(/\W/g, '');
...
function findLongestWord(sentence) {
  ...
  return newSentence.sort((a, b) => b.length - a.length)[0];
}
...
function extractNameFromTemplate(value) {
  ...
  const result = `${newValue[1]} ${newValue[2]}`;
...
function encodeToRot13(str) {
  ...
  const newLetters = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[a-z]/gi, (char) => newLetters[letters.indexOf(char)]);
}
